...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp, cp=1):
        ...
        self.sigma_paths = sigma_paths
        self.price = spot*np.exp(self.rho/self.vov*(sigma_paths[:,-1]-self.sigma)-self.rho**2*self.sigma**2*texp/2*I_t)
        self.sigmabs = np.sqrt((1-self.rho**2)*I_t)
        self.option_prices1 = self.bsm_formula(strike,self.price,texp,self.sigmabs)
...
class ModelNormalCondMC:
    ...
    def price(self, strike ,spot, texp, cp=1):
        ...
        self.price = spot+self.rho/self.vov*(self.sigma_paths[:,-1]-self.sigma_paths[:,0])
        self_option_prices1 = self.normal_formula(strike,self.price, texp, self.sigma_n)
