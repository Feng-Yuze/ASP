...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        for i in range( n_assets):
            a = forward[i] * np.exp(-cov_m[i,i] * texp/2 + np.sqrt(texp) * chol_m[i]@znorm_m) 
            forward_price.append(a)
        prices = np.array( forward_price )
...
def basket_price_norm_analytic(
    ...
    forward = ( spot * np.exp(texp*intr) * np.exp(-texp*divr) ) @ weights
    cov_m = vol * cor_m * vol[:,None]
    ...
    return  normal_formula(strike, forward, sigma, texp, intr=intr, divr=divr, cp_sign=1)
